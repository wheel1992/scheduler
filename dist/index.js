    ...
    run = (config) => {
        ...
        const instance = new croner_1.default(interval, {
            ...
            if (this.verbose) {
                ...
                console.info(`[Scheduler] Job: ${jobName}`, `Previous run: ${previousRunDate
                    ? previousRunDate.toISOString()
                    : '<no previous run>'}`, `Next run: ${nextRunDate ? nextRunDate.toISOString() : '<no next run>'}`);
            }
        });
